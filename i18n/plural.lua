local plural = {}
local default_function = nil
-- helper functions

local function assert_present_string(function_name, param_name, value)
  if type(value) ~= 'string' or #value == 0 then
    local msg = "Expected param %s of function %s to be a string, but got %s (a value of type %s) instead"
    error(msg:format(param_name, function_name, tostring(value), type(value)))
  end
end

local function assert_number(function_name, param_name, value)
...
local f3 = function(n)
  if not is_integer(n) then return 'other' end
...
local f11 = function(n)
  return ((n == 1 or n == 11) and 'one') or
         ...
         (is_integer(n) and (between(n, 3, 10) or between(n, 13, 19)) and 'few') or
         'other'
end
...
local f19 = function(n)
  return (n == 1 and 'one') or
         ((n == 0 or
          (n ~= 1 and is_integer(n) and between(n % 100, 1, 19)))
          and 'few') or
         'other'
end
...
local f24 = function(n)
  return (is_integer(n) and (n == 0 or n == 1 or between(n, 11, 99)) and 'one')
         or 'other'
end
pluralization[f24] = {'tzm'}

local pluralization_functions = {}
for f,locales in pairs(pluralization) do
  for _,locale in ipairs(locales) do
    pluralization_functions[locale] = f
  end
end
...
function plural.get(locale, n)
  assert_present_string('i18n.plural.get', 'locale', locale)
  assert_number('i18n.plural.get', 'n', n)

  local f = pluralization_functions[locale] or default_function

  return f(math.abs(n))
end

function plural.set_default_function(f)
  default_function = f
end

function plural.reset()
  default_function = pluralization_functions['en']
end
